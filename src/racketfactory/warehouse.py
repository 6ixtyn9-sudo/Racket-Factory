...
def enrich_live_card_with_api_odds(card: pd.DataFrame, target_date: str) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        # Scraped fallback requires a coherent two-way pair. A lone price
        # with no opposing side is usually a poisoned scrape (e.g. @9.50
        # parsed from page text), and the pair is the only orientation
        # evidence a label-only scrape carries.
