...
Recent changes
2026-09-19 — min-EV policy gap closed (session arena/01a0b2d6-racket-factory, on top of 8ccc420). The slice-export EV gate in mine_edges (`--min-ev`) defaulted to 0.0 while ml.py's `get_min_ev_real()` enforced the AGENT_PROMPT +2% floor via RACKET_FACTORY_MIN_EV — a +0.5% EV pick could be exported even though the policy floor was +2% (09-18's SKIPPED_DEAD_EDGE rows, EV −0.009/−0.011, would have survived a 0% gate by ~1u of luck). Fix: `--min-ev` default is now None, resolving through `get_min_ev_real()` — the SAME env var (RACKET_FACTORY_MIN_EV) and +2% default and fail-soft now govern both the slice-export gate and the ML capital-protection gate; an explicit `--min-ev` still wins as the diagnostics escape hatch. 3 new tests in tests/test_starvation_guards.py: a +1.6% EV row is SKIPPED_DEAD_EDGE by default (gate text "0.016 < 0.020"), exports WATCHLIST under RACKET_FACTORY_MIN_EV=0.01 (post-RED-DAY tuning restored in one line), and explicit `--min-ev 0.05` beats the env var. The two pre-existing 1%-pin tests now pin what their comments always claimed. 294/294 green. Verified the operator's one-line landing 8ccc420 (RACKET_FACTORY_EXPORT_NO_SLICE: '1' in daily.yml, clean single-insertion diff) — tomorrow's run will export no-slice candidates as WATCHLIST (marker no_slice_export) on would-be empty days, and the +2% gate now applies to them too.
2026-09-19 — No-pick day forensics (09-18 + 09-19) + no-slice diagnostics and opt-in export (session arena/01a0b2d6-racket-factory, on top of 4efaa06). Operator reported two consecutive zero-actionable-pick days (09-18: 3 rows, all skipped; 09-19: 81 candidates, 0 rows) and rejected "NO BET is correct" as the final answer. Diagnosis, production evidence: (1) 09-19 — all 81 candidates were live-injected rows; the 9 exportable slices mined that day (EDGE CONFIRMED CHALLENGER|1.3-1.6|High + Hard variant; WATCHLIST WTA|Hard|Both, Hard|High|Both, Hard|1.3-1.6|Both, Hard|High|ForeTennisOnly, CHALLENGER|1.3-1.6|High|MarketOnly + Hard variant) all require pred_confidence:High and/or multi-source agreement, and the slate missed them: ATP250-heavy, the only CHALLENGER rows were grass/no-prediction (Tiburon deep pass 0 parsed), ForeTennis 0 rows for the day, thin BetClan overlap. A local rebuild using the run's committed files reproduced the full pipeline (live card → warehouse merge → derivations → slice match) end-to-end: merge keys are stable under name_signature_strict, the match loop works, and 3 of 63 reconstructed rows (WTA|Hard|Both via betclan∩forebet overlap, e.g. De Stefano/Samson) SHOULD have matched — the residual 3-vs-0 is live-fetch composition (bzzoiro 402; forebet live 67 rows vs 75 in the file) and is not reproducible offline (run log URL expired). Not a pricing/EV/odds bug: 75/81 rows had live odds. (2) 09-18 — the 3 matched rows were legitimately gated: 1 SKIPPED_VETO (tour CHALLENGER + surface Hard veto), 2 SKIPPED_DEAD_EDGE (EV −0.009/−0.011 < 0.000 gate). (3) The real defect: silent zero days — 81 dropped rows left ONE warning log line as the entire operator-visible trace, and the live-only WATCHLIST fallback is structurally dead in this case (it fires only when NO actionable slices were mined — the opposite condition). Fix, three parts: (a) mine_edges.py now always writes picks_unmatched_<date>.json when candidates are dropped — per-row dims (tour/_series/_surface/pred_confidence/cross_source_agree/fav_odds_band) + confidence + selected-side odds + closest slice + missing dims (e.g. "tour=ATP!=CHALLENGER"); the dump is removed when a later run on the same date produces picks, and the warning now includes a tour split. (b) New operator dial RACKET_FACTORY_EXPORT_NO_SLICE (default OFF = validated-slice policy unchanged, existing test pinned): on would-be empty days only (actionable slices exist, slice pass exported nothing), the dropped candidates are exported through the same watchlist fallback exporter (EV gate, veto context, ML filter all still apply) with marker slice_matched=no_slice_export — it can never fire on a day that already has slice-matched picks (test-pinned). The fallback body was refactored into a shared _export_candidate_as_watchlist closure used by both recovery paths; the slice-match loop and the diagnostics share _dim_mismatches_slice so the series equivalences (International↔WTA250, Premier↔WTA500) have one rule. (c) daily.py renders a "NO-SLICE CANDIDATES (diagnostic — NOT bets)" section in the picks .txt from the dump (first 30 rows; full list in the JSON). 5 new tests in tests/test_starvation_guards.py (dump contents, stale-dump removal, opt-in export = WATCHLIST + EV + no dump, no-dilution on matched days, default-off behavior preserved). 291/291 green. Operator note: to get daily WATCHLIST coverage on no-match days, add RACKET_FACTORY_EXPORT_NO_SLICE: '1' to the workflow env (one line; these rows are WATCHLIST-tier, playable by auto_tickets like any WATCHLIST, but carry no validated edge — expect to judge them on the rolling audit). Known gap, deliberately not changed: mine_edges --min-ev still defaults to 0.0 (the workflow does not pass it) while ml.py's gate defaults to 2% per AGENT_PROMPT — the slice-export EV gate is looser than the stated policy (09-18's SKIPPED_DEAD_EDGE rows would survive a 0% gate only by ~1u).
2026-09-19 — Forebet deep pass: live-slug resolution (session arena/01a0b2d6-racket-factory). Root-caused and fixed the 15/15 dead tournament pages from run 35399503550. The URL template (/en/tennis/{tour_slug}/{tournament_slug}) was already correct — verified live: /en/tennis/atp-singles/us-open and /atp-singles/bastad are real "predictions" pages. What 404'd was slug DERIVATION: the deep pass slugified warehouse NAMES, and names drift from Forebet's canonical slugs — "Challenger Rennes Prediction" -> challenger-rennes-prediction (real: /challenger-men/rennes), "Challenger Guangzhou" -> guangzhou (real: guangzhou-2), "ATP Davis Cup - World Group II" -> atp-davis-cup-world-group-ii (real: davis-cup-world-group-ii), and the log's mysterious "us-open-116finals" is exactly slugify("US Open - 1/16-finals"). A real-data simulation of the committed September capture identified 24 distinct name-derived slugs in the 404 fleet (incl. unknown/wta-sao-paulo-116finals from a NaN tour). The authoritative slugs are the daily boards' match links (/tennis/matches/{tour_slug}/{tournament_slug}/...), which the parsers already extracted per row but which were dropped before writing. Fix, three parts: (1) mode_daily now persists tour_slug/tournament_slug into predictions_forebet_*.csv.gz (additive columns; the warehouse merge only reads 5 pred cols, so no pollution); (2) fetch_tournament_predictions accepts explicit tour_slug/tournament_slug (name derivation demoted to fallback); (3) mode_tournament now builds its fetch list from the current+previous month's predictions CSV (14-day recency window so active tournaments outrank stale early-month rows), joins live slugs onto warehouse rows by the (date, strict name-signature pair) identity, groups by resolved fetch-slug, ranks LIVE-slug groups first (verified pages) then name-derived fallbacks, busiest first, and skips empty-slug groups (the old groupby dropped NaN keys implicitly; the "" fallbacks don't). Also hardened forebet_tour_slug/forebet_tournament_slug against NaN (pandas 3.x str dtype keeps NaN through astype(str); the warehouse carries NaN tours) — this crash was caught in simulation, not tests. Cold start (captures predate slug persistence) falls back to the legacy name-derived path, capped by the 404 fail-fast. Verified: 287/287 tests (5 new: live-slug preference + count ranking, cold-start fallback, empty-slug group skip, loader month/window behavior, plus the 404-latch pair from yesterday); real-data simulation of the committed September capture -> top-15 fetch list = 13 verified-live pages first (cross-checked against the live hub nav fetched the same day) + 2 fallback slots. Scope note: the deep pass now targets CURRENTLY-ACTIVE tournaments; historical backfill of long-finished slugs stays out of scope (dead slugs 404; daily pages carry the results/settlement load).
2026-09-19 — Forebet relay hardening part 2: dead-slug 404 fail-fast (session arena/01a0b2d6-racket-factory). Run 35399503550 (JINA_API_KEY now set in the workflow) proved the relay itself is healthy — the daily pages came back full-size (tomorrow 318,846 bytes, 75 today matches / 68 stored new; 698 prediction rows written; BetExplorer 164 rows; warehouse 4103 rows / 74 live rows; bank 194.5% after the 09-17 slip won @4.08). The tournament deep pass (limit now 15, lowered 50→15 in daily.py to cut CF burst) still returned 0/15, but with a NEW signature: real Forebet 404 content pages for 12/15 slugs (stale warehouse-derived slugs like atp-bastad-prediction, atp-cincinnati-md, us-open-116finals) instead of CF shells — the key lets the relay reach Forebet, so Forebet now answers honestly: those slugs no longer exist. Each dead slug burned ~15s of relay time for a guaranteed empty board. Fix (same pattern as the CF latch): `forebet.py` now tracks `_notfound_hits`/`_notfound_blocked` — `_looks_like_board`'s 404 branch notes each real 404, 3 consecutive latch the run and `fetch_tournament_predictions` short-circuits with zero relay calls; any real board resets the streak (per-instance, per-run). Deliberately NOT recorded in the cross-run relay-status marker: dead slugs are a data-staleness state that changes as the tennis calendar rolls (new tournaments get fresh slugs), so a cross-run skip could suppress valid pages tomorrow, whereas a CF block is environmental and persists. `backfill_forebet.py`'s summary line now also appends `[warehouse slugs dead: 3+ consecutive 404 content pages — failing fast for this run]`. Expected: on stale-slug days the deep pass now costs ~3 pages × 15s instead of 15 × 15s, and the log line says why. 283/283 tests (2 new: latch+short-circuit, streak reset on a real board). Note for a future session: the deep pass's real problem is slug resolution (warehouse (tour, tournament) → live Forebet URL), not the relay — worth a dedicated pass only if the operator wants the deep pass back; the daily pages now carry the deepening load.
2026-09-18 — Forebet relay hardening: CF fail-fast latch + Slumdog alignment (session arena/01a0b2d6-racket-factory): (1) Run 35311356382 showed the tournament deep backfill burning all 50 pages on identical ~7.9KB Cloudflare "Just a moment..." shells that parsed to 0 silently (the shells echo the requested URL, so they passed the sport-label check). `forebet.py` now has a shared `_is_cf_challenge()` detector (replacing the 3 inline checks), every challenge snapshot logs a WARNING with the 200-char head, and 3 consecutive challenge snapshots latch `_relay_blocked` — `fetch_tournament_predictions` then short-circuits without a relay call; a real board resets the streak (per-instance, so the latch is per-run). `backfill_forebet.py`'s final line appends `[relay blocked by Cloudflare challenge — failing fast]`. Verified live in run 35312729591: latch fired at tournament 2/50, the remaining 48 skipped with zero relay calls (before: 50 pages × 2 fetches). (2) Slumdog comparison (same owner's repo, same r.jina.ai relay): Slumdog's committed capture receipts show the relay serving fresh ~151KB tennis boards from GitHub Actions every day with zero CF challenges — including its 2026-09-18 04:25Z run, 70 min before our first challenged fetch (05:36Z). Differences found: Slumdog's exact UA strings ("Slumdog" html mode, "EdgeFactory/1.0" reader mode — the Edge-Factory-validated set) and ~11 gentle relay calls/day vs our 3 + up to 100 deep-burst. Changes: relay UAs switched to Slumdog's validated set; new per-day relay health marker (`localdata/fetch_cache/forebet_relay_status_<day>.json`, gitignored/cached dir) — `--mode daily` records "challenged" when the predictor latches, and `--mode tournament` skips the 50-page burn entirely when today's marker is "challenged" (Slumdog discipline: a satellite that already failed its route is not re-attempted in the same run). 281/281 tests (5 new). If challenges persist after this lands, the block is forebet/CF-side against the relay's fetcher (Slumdog's next 04:30Z run is the canary) and needs operator-level egress, not code.
2026-09-18 — AGENT_PROMPT_ROBUST.md task execution (session arena/01a0b2d6-racket-factory, on top of 314f3ab): (1) BetExplorer two-anchor names: `_page_odds._resolve_link_names` now resolves one match rendered as two sibling anchors on the same href (`<a>Borisiouk M.</a> vs <a>Kim D. J.</a>`, the remaining bad_names case after the 2026-09-17 fused-name fix); `_match_link_count` dedupes by resolved identity so the box is not rejected as 2-match (before: rows lost as bad_names/no_box → after: parsed; 3 tests). All previously-failing text cases (Tiafoe F. Shelton B., Zverev A.Shelton B., Alcaraz Carlos Sinner Jannik, Van De Zandschulp) already pass — verified, no change. (2) OddsPortal upcoming: `_is_match_link` now accepts 2-seg `/tennis/<event-id>/` and 3-seg `/tennis/<cat>/<event-id>/` when the segment is ID-like (digit/uppercase), and `parse_listing_page` logs up to 5 rejected hrefs when a populated listing yields zero match links (the 2026-09-18 links=183/match_links=0 failure is now diagnosable from Actions logs without a live re-fetch). Live acceptance (oddsportal_upcoming_rows>0 3 days, cross_checked>30%) still needs an Actions run — no live egress from the sandbox. (3) Forebet tournament deep search: code (parse_page → parse_jina_markdown fallback both directions in fetch_tournament_predictions, daily.py threshold <0.60 over all predicted_winner* cols with coverage print) already present — verified offline with fixture tests proving the 0/50 relay-markdown case parses (2 tests); live >20/50 acceptance needs Actions. (4) Warehouse Bzzoiro merge: verified offline by building warehouse from committed localdata — predicted_winner_bzzoiro notna 1411 (>200 ✓), 2+ source predictions 664 rows/17.7% (>5% ✓), any-pred 41.3% (60%+ needs live backfill volume, not a code fix). No code change. (5) ML capital-protection gates per prompt: min EV default 0.01 → 0.02 and min per-leg odds base 1.20 → 1.30, both env-overridable (RACKET_FACTORY_MIN_EV / RACKET_FACTORY_MIN_ODDS restore the post-RED-DAY 1%/1.20 tuning in one line); BOOST legs still reach 1.15 with EV>=1%, doubles still need 5%, dynamic max odds 1.8→3.5 unchanged. 2 starvation-guards tests pin the 1% floor via env (they test the fallback, not the gate). (6) Pricing: REAL path (enrich_live_card → odds_source=BetExplorer → market_basis_for_pick api/non-paper) verified end-to-end with a new fixture test; the 2026-09-18 0/4-priced miss was live-data-side (comparison legs), which (1)+(2) address on the parse side. Full suite 276/276 green.
2026-09-18 - picks audit full-coverage fix (stale history pruning, carryover dedupe, bucket reconciliation): `localdata/picks_audit_2026-09-18.md` was not fully auditing the archived ledgers. Three defects: (1) `merge_audit_history` never pruned, so rows from a rewritten `picks_2026-09-17.json` (e.g. Da Silva Fick / Kulambayeva, S. De Stefano initials, Quevedo re-pick) resurrected in every future audit — 74 "archived rows" against 71 actual ledger rows; (2) a carryover re-pick archived in two daily ledgers (Mikulskyte / Smith in both 09-16 and 09-17, same selection) counted as two settled picks and credited two buckets, double-counting PnL (49 settled rows but only 48 unique matches); (3) By Tour/Series/Surface/Bucket/Source tables only covered settled picks, so pending picks (25) were invisible per bucket and all-pending groups (e.g. ATP/ATP250) vanished entirely. Fixes in `scripts/audit_recent_picks.py`: history merge now takes `valid_keys` from `archive_pick_keys()` (all archived pick rows, window-independent) and prunes rows of the current ledger kind whose pick no longer exists in any archived ledger, while history rows of other ledger kinds are preserved — this also fixes latent official/forecast cross-contamination through the shared `picks_audit_history.json` (rows now carry `ledger_kind`; reports filter to their own kind). `dedupe_by_identity()` collapses same match + same selection across daily ledgers (home/away order-insensitive), keeping the most-advanced-status then earliest-date row as canonical and recording losers in `duplicate_picks` (shown as `dup_merged=[...]` in the report). Group summaries (`summarize_by`) now take `all_rows` so every group reports `total_picks`/`pending_picks` in addition to settled wins/ROI, and all-pending groups still appear. New "Ledger reconciliation" section makes the accounting explicit: ledger rows − duplicates merged = audited rows, plus unresolved rows grouped by reason. Pending picks that find no settling row now get `near-miss candidates` in their reason when exactly one side of the pick matches a result row (single-side match only — settlement still requires the full teams_match), distinguishing resolvable name-variant matcher gaps (e.g. `J. Wolf vs L. E. Ambrogi` ↔ `Wolf J. vs Ambrogi L.` initials mismatch; bare `Burillo` vs compound `Burillo Escorihuela I.`) from real data gaps. The shared `settlement.py` matcher was intentionally left untouched (bankroll-critical); those gaps are surfaced, not auto-settled. Verified 2026-09-18: 71 ledger rows − 1 duplicate = 70 audited, 3 stale rows pruned, 48 unique settled (22 wins, hit 0.458333, ROI −0.246512 n=43), By Bucket totals reconcile exactly (SDE 36 = 25 settled + 11 pending; SV 8 = 3 + 5; WL 16 = 15 + 1; WNO 10 = 5 + 5). Narrow `--days 3` windows still report all history rows; official and forecast runs no longer share history. `PYTHONPATH=src python3 -m pytest -q` passed 264 tests (6 new in tests/test_audit_history.py).
