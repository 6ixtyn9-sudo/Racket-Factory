...
Recent changes
2026-06-29 - live odds side-alignment fix: Investigated the @9.50-style reports and found the two-way market pair itself can be valid, but assigned to the wrong home/away side during live scrape merge or reversed-source aggregation. No new files added. warehouse.py now re-orients reversed source rows, normalizes hyphenated player names for live grouping, and swaps likely side-inverted pairs such as 79% @9.50 / 21% @1.02 into 79% @1.02 / 21% @9.50 instead of rejecting the price. mine_edges.py now prices the actual selected side instead of fav_odds and can repair the same likely inversion from an older live warehouse row before EV math. Only incomplete/impossible live pairs are withheld into WATCHLIST_NO_ODDS. Regression coverage was added to existing tests/test_warehouse.py only.
